...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLAnchorElement>, index: number) => {
    if (e.key === 'Enter' || e.key === ' ') {
      ...
      const liEl = (e.currentTarget as HTMLElement).parentElement;
      if (liEl) {
        // Mock simple click event structure
        handleClick({ currentTarget: e.currentTarget, preventDefault: () => {} } as any, index);
      }
    }
  };
      ...
      <nav>
        <ul ref={navRef}>
          {items.map((item, index) => (
            <li key={index} className={`${activeIndex === index ? 'active' : ''} cursor-target`}>
